...
const MOCK_DATA = {
  ...
  routeTitle: "Direct Flight Pattern",
  connectionCount: 0,
  transitRegions: ["Direct"],
  estimatedDuration: "12h 30m",
  aiResponse: {
    visaOverview: {
      summary: "Japan offers visa-free entry for many nationalities for short-term tourism. However, specific requirements apply based on your passport and length of stay.",
      notes: [
        "Ensure your passport is valid for at least 6 months beyond stay.",
        "Check current Visit Japan Web requirements for digital customs/immigration.",
        "Proof of onward travel may be requested upon arrival."
      ]
    },
    documentationTips: [
      "Keep a digital and physical copy of your hotel reservations.",
      "Ensure you have travel insurance that covers international medical expenses.",
      "Download the 'Visit Japan Web' app and complete forms 24h before departure."
    ],
    transitLogic: {
      riskLevel: "Low",
      summary: "Direct flights eliminate transit visa risks. Japan's main entry points (NRT, HND) are efficient and well-documented.",
      notes: [
        "Direct routes are the safest for avoiding third-country visa complications.",
        "Both Narita and Haneda airports have 24/7 immigration facilities.",
        "Baggage will be handled directly to your final destination in Tokyo."
      ]
    },
    overallAssessment: {
      feasibility: "Highly Feasible: This is the most straightforward route for your selected destination and visa type.",
      finalNote: "This analysis is based on typical travel conditions. Always verify with official consulate websites."
    }
  }
};

export default function AITravelReport() {
  ...
  useEffect(() => {
    // Wait for user data to load before deciding on mock data
    ...
    if (savedAnalysis) {
      ...
      // If no data and NOT logged in, show mock data
      if (!userData?.data) {
        setData(MOCK_DATA);
        const timer = setTimeout(() => {
          setIsAnalyzing(false);
        ...
        // If logged in but no data, send back to dashboard
        router.push("/user-dashboard");
      }
    }
  }, [router, userData, isUserLoading]);

  const isPro = userData?.data?.subscriptionStatus === "ACTIVE";

  if (isAnalyzing || !data) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] container mx-auto">
  ...
  return (
    <div className="p-6 animate-in fade-in duration-700 container mx-auto">
