...
export default function TravelPatterns() {
    ...
    const handleContinue = async () => {
        ...
        // If user is not logged in, redirect to report page
        if (!userResponse?.data) {
            router.push("/traveling/visa-selection/typical-travel-routes/default-page");
            return;
        }
    ...
    return (
        <div className="min-h-screen flex justify-center p-6 bg-slate-50/30">
            <div className="w-full max-w-4xl rounded-xl">
                ...
                <a href="traveling/visa-selection/typical-travel-routes/ai-travel-report"></a>
                {/* Button */}
                <div className="flex justify-center mt-12 py-10">
                    <button 
                        onClick={handleContinue}
                        disabled={!selectedPattern || isAnalyzing}
                        className={`px-10 py-4 rounded-full font-bold transition-all shadow-lg flex items-center gap-2 relative overflow-hidden ${
                            selectedPattern 
